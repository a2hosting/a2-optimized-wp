...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	...
	class Requirements_Checker {
		...
		private function is_plugin_active( $a2_optimized, $plugin_slug ) {
			...
			$this->add_error_notice(
				$a2_optimized . ' is a required plugin.',
				$a2_optimized . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $a2_optimized A2 Optimized.
		...
		private function is_required_plugin_version_active( $a2_optimized, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$a2_optimized} {$min_plugin_version}+ is required.",
				"{$a2_optimized} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_a2_optimized ) {
					return $this->is_plugin_active( $required_a2_optimized, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_a2_optimized ) {
					return $this->is_required_plugin_version_active( $required_a2_optimized, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
